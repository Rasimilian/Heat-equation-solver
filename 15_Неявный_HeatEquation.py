...
l=200                       # кол-во слагаемых в ряде Фурье (в 2 раза меньше, т.к. сопряженные)
d_fi=2*pi/l                      ## l-кол-во слагаемых в разложении в ряд Фурье (в 2 раза меньше на самом деле)
signal=[func(x*d_fi) for x in range(0,l)]        # гран условие в виде массива
fourier_plus=np.fft.rfft(signal)*2/l           ## Нормировка неверная (2/l). Тут ряд фурье
real_fourier=[i.real for i in fourier_plus]      # реал часть
imag_fourier=[i.imag for i in fourier_plus]       # мним часть

# x={0,...,R}
# t={0,...,inf}
R=1 # Радиус диска
# R1=1
# u_x_0=[0 for i in range(0,int(l/2)+1)]           # гран условие в нуле реал часть
# u_x_0_1=[0 for i in range(0,int(l/2)+1)]         # гран условие в нуле мним часть
...
## Метод установления. Норма вектора решения в предпоследней итерации. Сумма модулей элементов (каждый узел сетки) комплексного числа roots[i]+j*roots1[i]
        ...
        matrix_d.reverse()
        matrix_d.append(roots[0]/t)
        matrix_d.reverse()
        null_gran_usl=roots[0]
        roots.append(u_x_n[m1])
        roots.reverse()
        roots.append(null_gran_usl)
        roots.reverse()
        ...
        matrix_d1.reverse()
        matrix_d1.append(roots1[0]/t)
        matrix_d1.reverse()
        null_gran_usl=roots1[0]
        roots1.append(u_x_n1[m1])
        roots1.reverse()
        roots1.append(null_gran_usl)
        roots1.reverse()
...
#                 solution_time1[m][w][i]=roots1[i]
    # assembling of Um(r,t) without m_angular part. Матрица без времени
    for i in range(0,n+1):
